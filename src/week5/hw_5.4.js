...
Note that you will need to probably change your projection to send more info through than just
that first character.
Also, you will need a filtering step to get rid of all documents where the city does not start
with a digital (0-9).
*/
...
db.zips.aggregate([
	{$project : {
		first_char: {$substr : ['$city', 0, 1]},
		city: 1, pop: 1, state: 1
	}},
	{$match: {'first_char': {$gte: '0', $lte: '9'}}},
	{$group: {_id : 0, total: {$sum : '$pop'}}}
]);
